...
def initializeProgram():
	...
	if __name__ == '__main__':
		...
		stderr.write(Fore.YELLOW + Back.RED + Style.BRIGHT +" ************************************************** ")
		stderr.write(Fore.YELLOW + Back.RED + Style.BRIGHT +" **  Please maximize the command prompt window.  ** ")
		stderr.write(Fore.YELLOW + Back.RED + Style.BRIGHT +" **        Then, press CTRL+C to continue.       ** ")
		stderr.write(Fore.YELLOW + Back.RED + Style.BRIGHT +" ************************************************** ")
...
def imageManip(image):
	...
	if (width%8) != 0:
		stderr.write(Fore.BLACK + Back.YELLOW + Style.NORMAL + " The width isn't a multiple of 8.")
	if (height%12) != 0:
		stderr.write(Fore.BLACK + Back.YELLOW + Style.NORMAL + " The height isn't a multiple of 12.")
	if (width%8) != 0 or (height%12) != 0:
		stderr.write(Fore.BLACK + Back.YELLOW + Style.NORMAL + " The image will be cropped. ")
	...
	while i < ((width*height)/(8*12)):
		stderr.write("\r" + Fore.BLUE + Back.WHITE + Style.NORMAL + " Averaging the colour of sample "+str(i+1)+"/"+str(int(((width*height)/(8*12))))+" of the input image. "+Fore.WHITE + Back.BLUE + Style.BRIGHT+" ("+str(int((i)/((width*height)/(8*12))*100))+"%) "+Fore.WHITE + Back.RED + Style.NORMAL+" This might take a while. ")
		l, a, b = getAverageLabColor(int(8*(i%(width/8))),int(12*int((8*i)/width)), 8, 12, image)
		input_grid.append((l,a,b))
		i += 1
	stderr.write("\r" + Fore.BLUE + Back.WHITE + Style.NORMAL + " Averaging the colour of sample "+str(i)+"/"+str(int(((width*height)/(8*12))))+" of the input image. "+Fore.WHITE + Back.BLUE + Style.BRIGHT+" ("+str(int((i)/((width*height)/(8*12))*100))+"%) "+Fore.WHITE + Back.RED + Style.NORMAL+" This might take a while. ")
...
def findBestCharacter(l, a, b, sample_number=1, total_samples=1, verbose=True):
	...
	for ref_char in charlist:
		...
		if verbose == True:
			stderr.write("\r" + Fore.BLUE + Back.WHITE + Style.NORMAL + " Comparing "+ eval('''Fore.'''+fg+''' + Back.'''+bg+''' + Style.'''+intensity+''' + \'\'\''''+char+'''\'\'\'''') + Fore.BLUE + Back.WHITE + Style.NORMAL+" "+str(permutation)+"/"+str(len(charlist))+" with sample "+str(sample_number)+"/"+str(total_samples)+". "+Fore.WHITE + Back.BLUE + Style.BRIGHT+" ("+str(int(((sample_number - 1)/total_samples+permutation/len(charlist)*1/total_samples)*100))+"%) "+Fore.WHITE + Back.RED + Style.NORMAL+" This WILL take a while. ")
		l, a, b = ref_char[1]
...
def printOutputTextParallel(image):
	...
	# sample_number = 0
	if __name__ == '__main__':
		stderr.write(Fore.BLUE + Back.WHITE + Style.NORMAL+" Matching each sample of the input image to a coloured character. ")
		starttime = datetime.datetime.now()
		findBestCharacterParallel((28.26223883969351, 49.90677151721382, 27.198472821976473))
		endtime = datetime.datetime.now()
		stderr.write(Fore.BLUE + Back.WHITE + Style.NORMAL+" Diving the workload between "+str(multiprocessing.cpu_count())+" workers. ")
		stderr.write(Fore.BLUE + Back.WHITE + Style.NORMAL+" Estimated time to complete: "+str((endtime - starttime)*len(grid)/int(multiprocessing.cpu_count()))+" ")
	...
	os.system('cls')
	stderr.write("")
	outfile = open("out"+str(os.getpid())+".ans", 'wb')
	outtext = ""
	print("")
	# print(len(output) == ((width*height)/(8*12)))
	for color in output:
		...
		if i % (width/8) == 0:
			outtext += "\n"
			print("")
		outtext += eval("Fore."+fg) + eval("Back."+bg) + eval("Style."+intensity) + char
		print(eval("Fore."+fg) + eval("Back."+bg) + eval("Style."+intensity) + char)
		i += 1
	outtext +="\x1B[0m\n"
	print("")
	outfile.write(bytes(outtext, "cp437"))
	outfile.close()
	if __name__ == '__main__':
